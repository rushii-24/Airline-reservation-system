...
# ✅ Email Config (Use App Password in production)
...
def book_flight(flight_id):
    ...
    if request.method == 'POST':
        ...
        booking_id = cursor.lastrowid  # This will fetch the last inserted ID
...
"""
        ...
        # Render the booking success template and pass the booking_id to the context
        return render_template('booking_success.html', 
                               flight=flight, 
                               name=name, 
                               booking_id=booking_id,  # Pass booking_id to the template
                               email=email)  # Include email in the context for the template
...
# ✅ E-Ticket Download Route (Using xhtml2pdf)
@app.route('/download_ticket/<int:booking_id>')
def download_ticket(booking_id):
    # Retrieve booking details from the database, including price
    cursor.execute("""
    ...
    # Render the HTML template for the e-ticket
    ticket_html = render_template('ticket_template.html', booking=booking)

    # Generate the PDF from the HTML using xhtml2pdf
    pdf = BytesIO()
    pisa_status = pisa.CreatePDF(ticket_html, dest=pdf)
